...
(function ($) {
	...
	function setPreloaderProgress(value) {
		...
		var percentText = preloader.find(".portfolio_loader_percent");
		preloaderProgress = progress;
		preloader.css("--loader-percent", progress);
		percentText.text(progress + "%");
		percentText.removeClass("is-ticking");
		window.setTimeout(function () {
			percentText.addClass("is-ticking");
		}, 10);
	}

	if ($(".preloader").length > 0) {
		setPreloaderProgress(4);
		var preloadAssets = Array.prototype.slice.call(document.images).filter(function (image) {
			return !image.closest(".preloader");
		});
		var loadedAssets = 0;
		var totalAssets = preloadAssets.length;

		function updateAssetProgress() {
			if (!totalAssets) {
				setPreloaderProgress(Math.min(92, preloaderProgress + 6));
				return;
			}

			setPreloaderProgress(Math.min(96, 6 + (loadedAssets / totalAssets) * 90));
		}

		preloadAssets.forEach(function (image) {
			if (image.complete) {
				loadedAssets += 1;
				return;
			}
		...
		preloaderTimer = window.setInterval(function () {
			if (preloaderProgress >= 88) {
				return;
			}

			setPreloaderProgress(preloaderProgress + 1);
		}, 180);
	}
	...
	/*------------------------------------------------------------------*/
	client_video_overlay: function() {
		if($('.client_video_tile video').length > 0){
			$('.client_video_tile video').each(function(){
				var $video = $(this);
				var $tile = $video.closest('.client_video_tile');
				...
				if (windscroll >= 0) {
					$('[data-scroll]').each(function(i) {
						if ($(this).position().top <= windscroll + 78) {
							...
							target.filter('[data-number="' + $(this).attr('data-scroll') + '"]').addClass('active');
						}
					});
				}else{
					target.removeClass('active');
	...
	$(window).on('load', function() {
		...
		$(".status").delay(240).fadeOut(900);
		$(".preloader").delay(520).fadeOut("slow");
	});
